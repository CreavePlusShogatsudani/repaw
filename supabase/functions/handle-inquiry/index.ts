...
// ワークスペースに紐づいていない API キーの場合は anthropic-workspace-id ヘッダーが必要
...
Q: 買取の流れを教えてください
A: Webフォームから申し込み → お手持ちの箱や袋に梱包して着払いで発送（送り先は申し込み完了画面とマイページに表示）→ 到着後に査定（2〜3営業日）→ マイページで査定結果のご確認 → 入金・寄付・返送の選択、という流れです。配送キットの送付はありません。
Q: 査定にはどのくらい時間がかかりますか？
A: 商品到着後、通常2〜3営業日以内に査定結果をメールでお知らせします。繁忙期は少しお時間をいただく場合があります。
Q: 査定額に納得できない場合は？
A: 査定額にご納得いただけない場合は、全点を着払いで返送するか、全点を寄付に回すかをお選びいただけます。買取できない服は、お申し込み時に選んだ方法（寄付または着払い返送）で対応します。
Q: 買取金額はどのように決まりますか？
A: ブランド、状態、需要、季節などをもとに、届いた商品を1点ずつ査定します。査定結果はマイページでご確認いただき、寄付・振込・返送をお選びいただけます。
Q: 複数点まとめて買取できますか？
A: はい、可能です。まとめて買取いただくことで、査定もスムーズに進みます。何点でもまとめてお送りいただけます。着払いのため送料のご負担はありません。
...
- Markdown 記法（** や # 、箇条書きの - など）は使わず、プレーンテキストで書く。手順は「1. 」「2. 」の番号と改行で示す
