  ...
  return (
    <section className="shop-container shop-size-section">
      ...
      <div className="shop-size-links">
        {['S', 'M', 'L', 'XL', 'フリーサイズ'].map(size => <Link key={size} to={`/products?size=${encodeURIComponent(size)}`} aria-label={`${size === 'フリーサイズ' ? size : `${size}サイズ`}の犬服を見る`}>{size === 'フリーサイズ' ? 'フリー' : size}<span aria-hidden="true">↗</span></Link>)}
      </div>
    </section>
  );
}
